...
taxa_table <- taxa_table %>%
                  ...
                  mutate(OTU = gsub('usearchOTU', 'OTU-', OTU)) %>%
                  distinct()
...
otu_count <- otu_count %>%
                ...
                mutate(Taxon = gsub('usearchOTU', 'OTU-', Taxon)) %>%
...
otu_w6_count  <- otu_w6_count[sampleSheet_w6$mouse.id,]
...
otu_w17_count <- otu_w17_count[sampleSheet_w17$mouse.id,]
...
otu_w24_count <- otu_w24_count[sampleSheet_w24$mouse.id,]
...
taxa_table  <- taxa_table[taxa_table$OTU %in% overlap_otu,]
...
### Keep OTUs with counts in more than 5% of samples
otu_w6_count  <- otu_w6_count[,colSums(otu_w6_count > 0) > (nrow(otu_w6_count) * .05)]
otu_w17_count <- otu_w17_count[,colSums(otu_w17_count > 0) > (nrow(otu_w17_count) * .05)]
otu_w24_count <- otu_w24_count[,colSums(otu_w24_count > 0) > (nrow(otu_w24_count) * .05)]
...
### DESeq2 VST
form <- formula(~ 1)
dds_wk6 <- DESeqDataSetFromMatrix(countData = t(otu_w6_count), colData  = sampleSheet_w6, design = form) 
...
form <- formula(~ diet)
dds_wk17 <- DESeqDataSetFromMatrix(countData = t(otu_w17_count), colData  = sampleSheet_w17, design = form) 
...
form <- formula(~ diet)
dds_wk24 <- DESeqDataSetFromMatrix(countData = t(otu_w24_count), colData  = sampleSheet_w24, design = form) 
...
annot.phenotype <- data.frame(data.name   = c(colnames(sampleSheet), taxa_table$OTU),
                              short.name  = c('Mouse Id', 'Sex', 'Diet', 'Week', 'Generation', 'Coat Color', 'Age at Sacrifice', 'Original Count ID', taxa_table$OTU),
                              R.name      = c(colnames(sampleSheet), taxa_table$OTU),
                              ...
                              units       = NA,
                              category    = c(rep('Demographic', ncol(sampleSheet)), rep('OTU Abundance', length(taxa_table$OTU))),
                              R.category  = c(rep('Demographic', ncol(sampleSheet)), rep('OTU Abundance', length(taxa_table$OTU))),
                              is.id       = c(TRUE, rep(FALSE, ncol(sampleSheet) - 1 + length(taxa_table$OTU))),
                              is.numeric  = c(rep(FALSE, ncol(sampleSheet)), rep(TRUE, length(taxa_table$OTU))),
                              is.date     = FALSE,
                              is.factor   = c(FALSE, FALSE, TRUE, rep(FALSE, ncol(sampleSheet) - 3 + length(taxa_table$OTU))),
                              factor.levels = c(rep(NA, 2), paste0(levels(sampleSheet_w17$diet), collapse = ':'), rep(NA, ncol(sampleSheet) - 3 + length(taxa_table$OTU))),
                              is.covar    = c(rep(FALSE, 2), TRUE, rep(FALSE, ncol(sampleSheet_w17) - 3 + length(taxa_table$OTU))),
                              is.pheno    = c(rep(FALSE, ncol(sampleSheet)), rep(TRUE, length(taxa_table$OTU))),
                              is.derived  = FALSE,
                              omit        = FALSE,
                              use.covar   = c(rep(NA, ncol(sampleSheet)), rep('diet', length(taxa_table$OTU))))
...
### Save
rm(list = ls()[!ls() %in% c(grep('dataset[.]', ls(), value = TRUE), 'genoprobs', 'map', 'markers', 'K')])
