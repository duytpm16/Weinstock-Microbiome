...
phylum_count <- read.xlsx(xlsxFile = "~/Desktop/Weinstock_Pomp_Microbiome/Phenotypes/16S/Raw Counts/taxonomic_abundance_filtered_unnormalized_20181004.xlsx", sheet = 'phylum_abundance')
...
phylum_w6_count  <- phylum_count %>% select(sampleSheet_w6$original.id)  %>% `colnames<-`(sampleSheet_w6$mouse.id[match(colnames(.),  sampleSheet_w6$original.id)])  %>% t(.)
phylum_w6_count  <- phylum_w6_count[sampleSheet_w6$mouse.id,]

phylum_w17_count <- phylum_count %>% select(sampleSheet_w17$original.id) %>% `colnames<-`(sampleSheet_w17$mouse.id[match(colnames(.), sampleSheet_w17$original.id)]) %>% t(.)
phylum_w17_count <- phylum_w17_count[sampleSheet_w17$mouse.id,]

phylum_w24_count <- phylum_count %>% select(sampleSheet_w24$original.id) %>% `colnames<-`(sampleSheet_w24$mouse.id[match(colnames(.), sampleSheet_w24$original.id)]) %>% t(.)
phylum_w24_count <- phylum_w24_count[sampleSheet_w24$mouse.id,]
...
overlap_phylum <- intersect(intersect(colnames(phylum_w6_count), colnames(phylum_w17_count)), colnames(phylum_w24_count))
taxa_table  <- taxa_table[taxa_table$phylum %in% overlap_phylum,]

phylum_w6_count  <- phylum_w6_count[,  taxa_table$phylum]
phylum_w17_count <- phylum_w17_count[, taxa_table$phylum]
phylum_w24_count <- phylum_w24_count[, taxa_table$phylum]



### Keep phylum with counts in more than 5% of samples
phylum_w6_count  <- phylum_w6_count[,colSums(phylum_w6_count > 0) > (nrow(phylum_w6_count) * .05)]
phylum_w17_count <- phylum_w17_count[,colSums(phylum_w17_count > 0) > (nrow(phylum_w17_count) * .05)]
phylum_w24_count <- phylum_w24_count[,colSums(phylum_w24_count > 0) > (nrow(phylum_w24_count) * .05)]
...
### DESeq2 VST
form <- formula(~ 1)
dds_wk6 <- DESeqDataSetFromMatrix(countData = t(phylum_w6_count), colData  = sampleSheet_w6, design = form) 
...
form <- formula(~ diet)
dds_wk17 <- DESeqDataSetFromMatrix(countData = t(phylum_w17_count), colData  = sampleSheet_w17, design = form) 
...
form <- formula(~ diet)
dds_wk24 <- DESeqDataSetFromMatrix(countData = t(phylum_w24_count), colData  = sampleSheet_w24, design = form) 
...
annot.phenotype <- data.frame(data.name   = c(colnames(sampleSheet), taxa_table$phylum),
                              short.name  = c('Mouse Id', 'Sex', 'Diet', 'Week', 'Generation', 'Coat Color', 'Age at Sacrifice', 'Original Count ID', taxa_table$phylum),
                              R.name      = c(colnames(sampleSheet), taxa_table$phylum),
                              ...
                              units       = NA,
                              category    = c(rep('Demographic', ncol(sampleSheet)), rep('Phylum Abundance', length(taxa_table$phylum))),
                              R.category  = c(rep('Demographic', ncol(sampleSheet)), rep('Phylum Abundance', length(taxa_table$phylum))),
                              is.id       = c(TRUE, rep(FALSE, ncol(sampleSheet) - 1 + length(taxa_table$phylum))),
                              is.numeric  = c(rep(FALSE, ncol(sampleSheet)), rep(TRUE, length(taxa_table$phylum))),
                              is.date     = FALSE,
                              is.factor   = c(FALSE, FALSE, TRUE, rep(FALSE, ncol(sampleSheet) - 3 + length(taxa_table$phylum))),
                              factor.levels = c(rep(NA, 2), paste0(levels(sampleSheet_w17$diet), collapse = ':'), rep(NA, ncol(sampleSheet) - 3 + length(taxa_table$phylum))),
                              is.covar    = c(rep(FALSE, 2), TRUE, rep(FALSE, ncol(sampleSheet_w17) - 3 + length(taxa_table$phylum))),
                              is.pheno    = c(rep(FALSE, ncol(sampleSheet)), rep(TRUE, length(taxa_table$phylum))),
                              is.derived  = FALSE,
                              omit        = FALSE,
                              use.covar   = c(rep(NA, ncol(sampleSheet)), rep('diet', length(taxa_table$phylum))))
...
### Qtl2 viewer format
dataset.phylum.w6 <- list(annot.phenotype = as_tibble(annot.phenotype),
                          annot.samples   = as_tibble(sampleSheet_w6),
                          covar.matrix    = covar.w6,
                          covar.info      = NULL,
                          data = list(raw = phylum_w6_count,
                                      vst = vst_wk6,
                                      rz  = rz_wk6),
                          taxa = taxa_table,
                          datatype = 'phenotype',
                          display.name = 'Phylum - Week 6',
                          lod.peaks = list())




dataset.phylum.w17 <- list(annot.phenotype = as_tibble(annot.phenotype),
                           annot.samples   = as_tibble(sampleSheet_w17),
                           covar.matrix    = covar.w17,
                           covar.info      = covar.info,
                           data = list(raw = phylum_w17_count,
                                       vst = vst_wk17,
                                       rz  = rz_wk17),
                           taxa = taxa_table,
                           datatype     = 'phenotype',
                           display.name = 'Phylum - Week 17',
                           lod.peaks    = list())
...
dataset.phylum.w24 <- list(annot.phenotype = as_tibble(annot.phenotype),
                           annot.samples   = as_tibble(sampleSheet_w24),
                           covar.matrix    = covar.w24,
                           covar.info      = covar.info,
                           data = list(raw = phylum_w24_count,
                                       vst = vst_wk24,
                                       rz  = rz_wk24),
                           taxa = taxa_table,
                           datatype     = 'phenotype',
                           display.name = 'Phylum - Week 24',
                           lod.peaks    = list())
